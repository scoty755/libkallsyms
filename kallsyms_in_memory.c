...
unsigned long
kallsyms_in_memory_lookup_name(kallsyms *info, const char *name, int is64bit)
{
    char namebuf[1024];
    unsigned long i;
    unsigned int off;
    ...
    for (i = 0, off = 0; i < info->num_syms; i++) {
        ...
        if (strcmp(namebuf, name) == 0) {
            if (is64bit == 1) {
                return le64toh(info->addresses[i]);
            } else {
                                ...
                                unsigned long *addresses, size_t n_addresses, int is64bit)
{
    ...
    for (i = 0, off = 0, count = 0;
        ...
        if (strcmp(namebuf, name) == 0) {
            if(is64bit == 1) {
                addresses[count] = le64toh(info->addresses[i]);
            } else{
...
const char *
kallsyms_in_memory_lookup_address(kallsyms *info, unsigned long address, int is64bit)
{
    ...
    for (i = 0, off = 0; i < info->num_syms; i++) {
        off = kallsyms_in_memory_expand_symbol(info, off, namebuf);
        if (is64bit == 1) {
            if (le64toh(info->addresses[i]) == address) {
                return namebuf;
            }
        } else {
...
static const unsigned long const pattern_kallsyms_addresses_7[PATTERN_MAX] = {
    0xffffffc000080000, // _text
    0xffffffc000080040, // stext
    0xffffffc000080080, // __h_error
    0xffffffc000080080, // __h_error_p
...
static int
get_kallsyms_in_memory_addresses(kallsyms *info, unsigned long *mem, unsigned long length, unsigned long offset, int is64bit)
{
    ...
    while (addr < end) {
        ...
        for (i = 0; i < sizeof (pattern_kallsyms_addresses) / sizeof (pattern_kallsyms_addresses[0]); i++) {
            ...
            for (j = 0; j < PATTERN_MAX; j++) {
                if (is64bit == 1) {
                    pattern_le[j] = htole64(pattern_kallsyms_addresses[i][j]);
                } else {
        ...
        unsigned long n=0;
        if (is64bit == 1) {
            while (le64toh(addr[0]) <= 0xffffffc000080000LL) {
                n++;
                addr++;
                if (addr >= end) {
                    return 0;
...
kallsyms *
kallsyms_in_memory_init(unsigned long *mem, size_t len, int is64bit)
{
    kallsyms *info;
    ...
    int ret = get_kallsyms_in_memory_addresses(info, mem, len, mmap_offset, is64bit);
    if (!ret) {
...
void
kallsyms_in_memory_print_all_to_file(kallsyms *info, FILE *fp, int is64bit)
{
    char namebuf[1024];
    unsigned long i;
    unsigned int off;
    ...
    for (i = 0, off = 0; i < info->num_syms; i++) {
        off = kallsyms_in_memory_expand_symbol(info, off, namebuf);
        if (is64bit == 1) {
            fprintf(fp, "%08lx %s\n", (unsigned long)htole64(info->addresses[i]), namebuf);
        } else {
            fprintf(fp, "%08lx %s\n", (unsigned long)htole32(info->addresses[i]), namebuf);
        }
    }
    return;
}
...
void
kallsyms_in_memory_print_all(kallsyms *info, int is64bit)
{
    if (!info) {
        return;
    }
    kallsyms_in_memory_print_all_to_file(info, stdout, is64bit);
}
